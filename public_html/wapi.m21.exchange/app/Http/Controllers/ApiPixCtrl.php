...
class ApiPixCtrl extends Controller
{
    ...
    function Transactions(Request $request)
    {
       ...
       $user_id= Auth::user()->id;
            ...
            $deposits = DB::table('deposit_address')->select('id','transactionId','amount','user_id','status',  'created_at')->where('user_id', $user_id)->get();
...
$a=file_get_contents("https://alphakoin.conectate.club/amazon.php?InvoiceID=".$InvoiceID);
...
if($ar->status=="Completed")
{
     ...
     $deposits = DB::table('deposit_address') ->where('status', "Created")->where('id', $InvoiceID)->first();
...
$amount=$ar->amount;
   ...
   $res=   $pl->credit_user( $user ,  $amount, "BRL"  ,'Deposit by  Pix invoice id '.$InvoiceID  );
   
   
    
 
  $id2 =  DB::table('deposit_address')
            ->where('id', $InvoiceID)
            ->update([  'status' =>  "Completed"     ,'url_res' =>$a  ]);
