...
class AuthController extends Controller
{
 ...
 $user = User::where('email',$this->request->email)->where('code', $this->request->code)->first();
    ...
    ['username' => $user->username ,
          'user_id' => $user->id,  'location' =>$location_array->country_name,
          'ip'=>$ip]
);
 ...
 public   function get_client_ip2() {
    ...
    public function send_otp1 (Request $request) {
        ...
        $otp=12335;
            $data = array( 'content'=>'OTP for the registeration is '. $otp,'subject'=>  'OTP for the registeration is '. $otp );
       ...
       $lg= new OTP();
        ...
        $lg->email=$email;
         ...
         $message->to($email,  $email)->subject
            ('OTP for the registeration is '.   $otp );
