...
$user =DB::table('users') ->where("BRL",'>',0) ->first()  ;
 ...
 print_r($user);
    ...
    $pl->convert_currency($user, "DT03072022" , "BRL"  );
         
         
         
      
$user =DB::table('users')->select("BRL","id","email")->where( "id",$user->id) ->first()  ;
 
 print_r($user);
 
 
         
        //   $pl->credit_user( $user ,  $user->BRL,  $coin  ,'DBIK deposit after currency auto conversion ' d  , "DT03072022"  );
...
function   private_blockchian_deposit(Request $request){
 ...
 $system_transactions=DB::table('wallet_transactions')->where("reference",$txid)->first()  ;
...
$address=$_REQUEST['address'];
 ...
 $wallet=DB::table('private_bnext_wallet')->where("address",$address)->first()  ;
...
$coin="DBIK";
 ...
 echo __LINE__;
    ...
    $pl=new Paymentcls();
   $pl->credit_user( $user ,  $amount,$coin  ,'DBIK deposit '.$txid  , $txid  );
         
    
// $user=DB::table('users')->where("id", $wallet->user_id   ) ->first()  ;
 
//file_put_contents($file,print_r($user,true), FILE_APPEND | LOCK_EX);
 ...
 // return $system_transactions;
  ...
  {
      $file = __LINE__.'people.txt';

$input = $this->request->all();


file_put_contents($file, print_r($input,true), FILE_APPEND | LOCK_EX);
...
file_put_contents($file,$address, FILE_APPEND | LOCK_EX);

      
 $data=DB::table('private_bnext_wallet')->where("address", $address   ) ->first()  ;
   ...
   $pl=new Paymentcls();
    $amount=$input['amount'];
      $coin="DBIK";
         ...
         $res=   $pl->credit_user( $user ,  $amount,$coin  ,'Deposit via transaction id '.$input['txid']  );
...
$user->save();
 ...
 $st->description = "Investment of ".$o->amount." for the package ".$o->package_name ;
    ...
    $users = DB::table('wallet_transactions')->where('package_name',  "Package one")->orderBy('id','desc')->count();
