...
{ 
  $user_id= Auth::user()->id;
    ...
    //  $user_roll=22;
     ...
     // echo $user_roll;
    if($user_roll >= 10){
      $data=DB::table('deposit_address')->select("id","amount","status","transactionId","cttimestamp")->where('coin', "BRL")->orderBy('id', 'desc')->get()  ;   
    }
    else{
 $data=DB::table('deposit_address')->select("id","amount","status","transactionId","cttimestamp")->where('user_id',$user_id)->where('coin', "BRL")->orderBy('id', 'desc')->get()  ;

    }
    
    
    
    //  $data=DB::table('deposit_address')->where('coin', "BRL")->orderBy('id', 'desc')->take(35)->get()  ;   
